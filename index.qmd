...
*Last updated on `r format(Sys.time(), "%Y-%m-%d %H:%M %Z")`*
...
datasets <- data %>%
  rename(export_modified = modified) %>%
  mutate(
    api_data = map(dataset_key, get_created_modified)
  ) %>%
  unnest(api_data, keep_empty = TRUE) %>%
  mutate(
